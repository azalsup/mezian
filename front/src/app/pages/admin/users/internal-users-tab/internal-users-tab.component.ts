...
@Component({
  ...
  templateUrl: './internal-users-tab.component.html',
})
export class InternalUsersTabComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.load();
    this.adminApi.listRoles().subscribe({ next: r => this.roles.set(r) });
  }

  load(): void {
    this.loading.set(true);
    this.adminApi.listUsers(this.page, this.pageSize, 'internal').subscribe({
